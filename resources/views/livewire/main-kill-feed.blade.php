<div class="overflow-hidden rounded-lg border border-gray-200 bg-white shadow-sm dark:border-white/10 dark:bg-gray-900">
    ...
    <div class="overflow-x-auto">
        <table class="min-w-full divide-y divide-gray-200 dark:divide-white/10">
            <thead class="bg-gray-50 dark:bg-gray-800">
                <tr>
                    <th scope="col" class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500 dark:text-gray-300">Date / Time (UTC)</th>
            ...
            <tbody class="divide-y divide-gray-200 bg-white dark:divide-white/10 dark:bg-gray-900">
                @forelse ($kills as $kill)
                    ...
                    <tr style="{{ $killType !== \App\Models\Kill::TYPE_VEHICLE ? 'background-color: indigo' : '' }}">
                        <td class="whitespace-nowrap px-6 py-4 text-sm text-gray-900 dark:text-gray-100">{{ \Carbon\Carbon::parse($kill->destroyed_at)->format('D, M j Y H:i') }}</td>
                        ...
                        @if($victimOrg->spectrum_id !== \App\Models\Organization::ORG_NONE && $victimOrg->spectrum_id !== \App\Models\Organization::ORG_REDACTED)
                        ...
                        @else
                            <td class="whitespace-nowrap px-6 py-4 text-sm text-gray-500 dark:text-gray-300"><img width="50" height="50" src="{{ $victimOrgIcon }}" alt="{{ $victimOrg->name }}" /></td>
                        @endif
                        ...
                        @if($killerOrg->spectrum_id !== \App\Models\Organization::ORG_NONE && $killerOrg->spectrum_id !== \App\Models\Organization::ORG_REDACTED)
    ...
    <div class="border-t border border-gray-200 bg-white shadow-sm dark:border-white/10 dark:bg-gray-900 px-4 py-3 sm:px-6">
        {{ $kills->links() }}
    </div>
</div>
