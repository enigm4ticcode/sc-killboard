<div class="card p-4 sm:p-6 lg:p-8 mx-4 sm:mx-0">
    <header class="pb-6 mb-6 border-b sc-hud-header sc-grid-pattern sc-hud-glow" style="border-color: rgb(var(--card-border));">
        <div class="flex flex-col sm:flex-row items-start sm:items-center gap-4 sm:gap-6">
            <div class="shrink-0">
                <a href="https://robertsspaceindustries.com/orgs/{{ $organization->spectrum_id }}" target="_blank" rel="noopener noreferrer" class="inline-block">
                    <img class="h-24 w-24 sm:h-32 sm:w-32 rounded-lg object-cover shadow-lg transition-all" src="{{ $organization->icon }}" alt="{{ $organization->name }}">
                </a>
            </div>
