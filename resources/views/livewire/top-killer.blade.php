<div class="card !rounded-xl overflow-hidden">
    <div class="border-b px-4 py-3 text-sm font-bold sc-hud-header sc-grid-pattern sc-hud-glow" style="border-color: rgba(var(--accent), 0.3); background: linear-gradient(135deg, rgba(34, 197, 94, 0.15) 0%, rgba(34, 197, 94, 0.05) 100%); color: rgb(var(--fg));">
        <div class="flex items-center gap-2">
            <span class="text-xl">🏆</span>
            <span>{{ __('app.top_killers', ['count' => config('killboard.leaderboards.number-of-positions'), 'days' => config('killboard.leaderboards.timespan-days')]) }}</span>
        </div>
    </div>
        ...
        <table class="min-w-full divide-y" style="divide-color: rgb(var(--card-border));">
            ...
            @forelse ($killers as $i => $killer)
                <tr class="transition-colors" style="background-color: rgba(34, 197, 94, 0.08);" onmouseover="this.style.backgroundColor='rgb(var(--table-hover))'" onmouseout="this.style.backgroundColor='rgba(34, 197, 94, 0.08)'">
                    ...
                    @if (! empty($killer->killer->avatar) && (str_contains($killer->killer->avatar, '/media/') || str_contains($killer->killer->avatar, '/static/images/account/avatar_default')))
                    ...
                    @else
                        <td class="px-2 py-3 text-center align-middle">
                            <a href="{{ route('player.show', ['name' => $killer->killer->name]) }}" class="inline-block transition-transform hover:scale-110">
                                <img width="48" height="48" src="https://cdn.robertsspaceindustries.com/static/images/account/avatar_default_big.jpg" alt="{{ $killer->killer->name }}" class="mx-auto align-middle rounded-full w-12 h-12 object-cover" style="box-shadow: 0 4px 8px rgba(0, 0, 0, 0.15);" />
                            </a>
                        </td>
                    @endif
                    ...
                    <td class="px-2 py-3 text-right align-middle whitespace-nowrap">
                        <span class="inline-block px-2 py-1 text-xs font-bold rounded" style="color: rgb(var(--fg)); background-color: rgba(34, 197, 94, 0.2);">
                            {{ $killer->kill_count }}
                        </span>
                    </td>
                </tr>
