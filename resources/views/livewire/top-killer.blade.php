            ...
            @forelse ($killers as $i => $killer)
                <tr class="bg-green-200/20 dark:bg-green-900/20">
                    ...
                    <td class="px-6 py-4 text-sm text-gray-500 dark:text-gray-300 align-middle whitespace-nowrap">
                        <b><a href="{{ route('player.show', ['name' => $killer->killer->name]) }}">{{ $killer->killer->name }}</a></b>
                    </td>
                    <td class="px-6 py-4 text-sm text-gray-700 dark:text-gray-200 text-right align-middle whitespace-nowrap font-mono">{{ $killer->kill_count }}</td>
                </tr>
            @empty
                <tr>
