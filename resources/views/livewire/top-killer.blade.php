<div class="overflow-hidden rounded-lg border border-gray-200 bg-white shadow-sm dark:border-white/10 dark:bg-gray-900">
    <div class="border-b border-gray-200 bg-gray-50 px-4 py-3 text-sm font-medium text-gray-700 dark:border-white/10 dark:bg-gray-800 dark:text-gray-200">
        Top {{ config('killboard.leaderboards.number-of-positions') }} Ship Killers (Last {{ config('killboard.leaderboards.timespan-days') }} Days)
    </div>
            ...
            @forelse ($killers as $i => $killer)
                <tr class="bg-green-200/20 dark:bg-green-900/20">
                    ...
                    @if ($killer->killer->avatar !== null)
                        <td class="whitespace-nowrap px-6 py-4 text-sm text-gray-500 dark:text-gray-300"><a href="{{ route('player.show', ['name' => $killer->killer->name]) }}"><img width="50" height="50" alt="{{ $killer->killer->name }}" src="{{ $killer->killer->avatar }}" /></a></td>
                    @else
                    ...
                    @endif
                    <td class="whitespace-nowrap px-6 py-4 text-sm text-gray-500 dark:text-gray-300"><b><a href="{{ route('player.show', ['name' => $killer->killer->name]) }}">{{ $killer->killer->name }}</a></b></td>
