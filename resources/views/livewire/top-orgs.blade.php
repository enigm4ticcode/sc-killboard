<div class="card !rounded-xl overflow-hidden">
    <div class="border-b px-4 py-3 text-sm font-bold" style="border-color: rgba(var(--accent), 0.3); background: linear-gradient(135deg, rgba(var(--accent), 0.15) 0%, rgba(var(--accent), 0.05) 100%); color: rgb(var(--fg));">
        <div class="flex items-center gap-2">
            ...
            <span>{{ __('app.top_org_killers', ['count' => config('killboard.leaderboards.number-of-positions'), 'days' => config('killboard.leaderboards.timespan-days')]) }}</span>
        </div>
    </div>
            ...
            @forelse ($orgs as $i => $org)
                @if($org->spectrum_id !== \App\Models\Organization::ORG_NONE)
                    <tr class="transition-colors" style="background-color: rgba(var(--accent), 0.05);" onmouseover="this.style.backgroundColor='rgb(var(--table-hover))'" onmouseout="this.style.backgroundColor='rgba(var(--accent), 0.05)'">
                        ...
                        <td class="px-2 py-3 text-xs text-right align-middle whitespace-nowrap font-mono" style="color: rgb(var(--muted));">{{ \Illuminate\Support\Number::format($org->average_kills_per_player, 1) }}</td>
                    </tr>
                @endif
            @empty
                <tr>
