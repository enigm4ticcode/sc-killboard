            ...
            @forelse ($victims as $i => $victim)
                <tr class="bg-green-200/20 dark:bg-green-900/20">
                    ...
                    @if ($victim->victim->avatar !== null)
                        <td class="whitespace-nowrap px-6 py-4 text-sm text-gray-500 dark:text-gray-300"><a href="{{ route('player.show', ['name' => $victim->victim->name]) }}"><img width="50" height="50" alt="{{ $victim->victim->name }}" src="{{ $victim->victim->avatar }}" /></a></td>
                    @else
                    ...
                    @endif
                    <td class="whitespace-nowrap px-6 py-4 text-sm text-gray-500 dark:text-gray-300"><b><a href="{{ route('player.show', ['name' => $victim->victim->name]) }}">{{ $victim->victim->name }}</a></b></td>
